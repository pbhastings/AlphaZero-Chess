...
#the value head predicts on [-1,1], so we need to scale this to some similar fixed range.
#can use tanh if we first condense the range of eval_board. divide by some fixed constant.
def eval_board(board: chess.Board):
    ev = 0
    for i in range(64):
        if (board.piece_at(i) is not None):
            ev += eval_piece(i, board.piece_at(i))
    eval = math.tanh(ev/c.EVAL_SCALAR) #this is positive if white is winning, so we convert to current player
